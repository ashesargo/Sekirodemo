...
public class DangerousAttackConverter : MonoBehaviour
{
    ...
    private DangerFX dangerFX = null;
    ...
    void Update()
    {
        if ( danger == null )
        {
            danger = GameObject.FindWithTag("Danger");
            dangerFX = danger.GetComponent<DangerFX>();
        }
    ...
    public void OnDangerousAttackStart()
    {
        ...
        // 播放危攻擊特效
        dangerFX.TriggerDangerEffect();
        DangerJump(jump);
    }
    //危可跳
    public void DangerJump(GameObject target)
    {
        if (target == null)
        {
        ...
        StartCoroutine(ToggleCoroutine(target));
    }

    private IEnumerator ToggleCoroutine(GameObject target)
    {
        // 啟用物件
